...
const createPages = () => {
  ...
  if (hasPrevFrau) {
    str += "<li class='page-item previous no'><a href='/admin/tipos/fraudes?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarTipoBox').value + "&dir=prev' class='nav-link'>&#9664 Anterior</a>";
  } else {
    str += "<li><a href='#' class='nav-link disabled'>&#9664 Anterior</a>";
  }

  if (hasNextFrau) {
    str += "<li class='page-item next no'><a href='/admin/tipos/fraudes?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarTipoBox').value + "&dir=next' class='nav-link'>Siguiente &#9654</a>";
  } else {
...
elemNew.setAttribute('href', `/admin/tipos/fraudes/add?part=${getCookie('filtro')}`)

const elemDel = document.getElementById('del');
elemDel.setAttribute('action', `/admin/tipos/fraudes/delete?part=${getCookie('filtro')}`)
