const buildEventosTable = (state) => {
  const table = document.getElementById('tblEventos')
  ...
  myList.map(element => {
    ...
    // col4
    cell = document.createElement('td')
    if (element.OBSEVE !== null) {
      cell.innerHTML = `<div class="d-flex py-1 align-items-center">
        <div class="flex-fill">
          <div class="font-weight-medium">${element.OBSEVE.length > 30 ? element.OBSFRA.slice(0, 30) + '...' : element.OBSFRA}</div>
        </div>
      </div>`
    }
    row.appendChild(cell)
    ...
    cell.innerHTML = `<ul class="dots-menu">
      <li class="nav-item drop-right">
        ...
        <ul>
          <li class="nav-item">
            <a href="/admin/fraudes/edit/${element.IDFRAU}/${element.IDEVEN}" class="nav-link">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
          ...
          <li class="nav-item">
            <a href="#" class="nav-link" onclick="{document.getElementById('evebor').value ='${element.IDEVEN}', document.getElementById('msgbor').innerHTML ='<p>${element.STRFEC}</p><p>${element.DESTIP}</p>'}" data-bs-toggle="modal" data-bs-target="#modal-borrar">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
