...
const buildTable = (state) => {
  ...
  myList.map(element => {
    ...
    cell.innerHTML = `<div class="d-flex py-1 align-items-center">
      <div class="flex-fill">
        <div class="font-weight-medium">${element.CODOFI}</div>
      </div>
    </div>`
    row.appendChild(cell)
    ...
    cell.innerHTML = `<ul class="dots-menu">
      <li class="nav-item drop-right">
        ...
        <ul>
          ...
          <li class="nav-item">
            <a href="#" class="nav-link" onclick="{document.getElementById('idofic').value ='${element.IDOFIC}', document.getElementById('msgbor').innerHTML ='<p>${element.DESOFI}</p><p>${element.CODOFI}</p>'}" data-bs-toggle="modal" data-bs-target="#modal-borrar">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
